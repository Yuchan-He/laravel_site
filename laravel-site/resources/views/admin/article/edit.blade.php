...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> Home <span class="c-gray en">&gt;</span> 文章管理 <span class="c-gray en">&gt;</span> 文章編集 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="更新" ><i class="Hui-iconfont">&#xe68f;</i></a>
</nav>
	...
	<form class="form form-horizontal" id="form-article-add" method="post">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$model->title}}" placeholder="" id="articletitle" name="title">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"></span>文章摘要：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="desn" cols="" rows="" class="textarea"  placeholder="" datatype="*10-100" dragonfly="true" nullmsg="備考をご記入ください！">{{$model->desn}}</textarea>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="body" cols="" rows="" class="textarea"  placeholder="何か書きましょう..." datatype="*10-100" dragonfly="true" nullmsg="備考をご記入ください">{{$model->body}}</textarea>
				<p class="textarea-numberbar"></p>
			</div>
		</div>
...
<!-- 				<button onClick="article_save();" class="btn btn-secondary radius" type="button"><i class="Hui-iconfont">&#xe632;</i> 保存草稿</button> -->
				<button class="btn btn-default radius" type="button"><a href="{{route('admin.article.index')}}">&nbsp;&nbsp;キャンセル&nbsp;&nbsp;</a></button>
			</div>
		</div>
	</form>
</article>
	...
	$("#form-article-add").validate({
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
				// type: 'PUT',
				// 自己提交给自己，不需要指定
				url: "{{route('admin.article.update',['id' => $model->id])}}" ,
				success: function(data){
					layer.msg('文章を更新しました!',{icon:1,time:2000},function(){
                ...
                error: function(XmlHttpRequest, textStatus, errorThrown){
					layer.msg('文章の更新失敗しました',{icon:2,time:3000},function(){
